...
def main() -> None:
    ...
    exit_position = ExitPosition(x=0.0, y=-300.0, z=3000.0)

    shape = (120, 160)
    baseline = np.full(shape, 2200, dtype=np.uint16)

    clear_engine = OccupancyEngine(zone, shape, exit_position)
    assert clear_engine.bounds.x_min == -450.0
    assert clear_engine.bounds.x_max == 450.0
    assert clear_engine.bounds.y_min == -1300.0
    assert clear_engine.bounds.y_max == 700.0
    assert clear_engine.bounds.z_min == 200.0
    assert clear_engine.bounds.z_max == 3000.0
    assert zone.monitored_depth_mm == 2800
    ...
    tiny_engine = OccupancyEngine(zone, shape, exit_position)
    ...
    blocked_engine = OccupancyEngine(zone, shape, exit_position)
    ...
    invalid_engine = OccupancyEngine(zone, shape, exit_position)
